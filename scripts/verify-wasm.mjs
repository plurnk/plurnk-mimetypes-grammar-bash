#!/usr/bin/env node
// CI byte-identical reproducibility check for bash.wasm.
...
const committed = await readFile(path.join(repoRoot, "bash.wasm"));
const committedHash = createHash("sha256").update(committed).digest("hex");
console.log(`committed bash.wasm sha256: ${committedHash}`);

const work = await mkdtemp(path.join(tmpdir(), "grammar-bash-verify-"));
...
console.log(`rebuilt bash.wasm sha256: ${rebuiltHash}`);
