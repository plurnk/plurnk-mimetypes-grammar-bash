#!/usr/bin/env node
// Reproducible WASM build for tree-sitter-bash.
...
const work = await mkdtemp(path.join(tmpdir(), "grammar-bash-build-"));
await run("git", ["clone", "--no-checkout", "https://github.com/tree-sitter/tree-sitter-bash.git", "src"], { cwd: work });
...
// Locate produced wasm and copy as bash.wasm.
const fs = await import("node:fs/promises");
const built = (await fs.readdir(buildCwd)).find((f) => f.endsWith(".wasm"));
if (!built) throw new Error("no .wasm produced");
await copyFile(path.join(buildCwd, built), path.join(repoRoot, "bash.wasm"));
const bytes = (await readFile(path.join(repoRoot, "bash.wasm"))).length;
console.log(`bash.wasm: ${bytes} bytes (built from ${pin})`);
